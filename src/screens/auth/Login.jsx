...
export function LoginScreen({ nav, onLoginSuccess }) {
    ...
    <div className="re-screen re-screen-enter">
      ...
      <div className="re-scroll re-pad" style={{ paddingBottom: 32 }}>
        <div style={{ marginTop: 4, marginBottom: 26 }}>
          ...
          <p style={{ fontSize: 14, color: "var(--text-dim)", marginTop: 8 }}>Inicia sesión para administrar tu negocio.</p>
        </div>
          ...
          <div className="re-banner-error" style={{ marginBottom: 18 }}>
            ...
            <div>
              ...
              {error.code === "EMAIL_NOT_VERIFIED" && (
                <div><a onClick={() => nav.go("verifyEmail", { email })} style={{ color: "var(--error-text)", fontWeight: 700, textDecoration: "underline" }}>Reenviar verificación</a></div>
              )}
              {error.code === "EMAIL_NOT_FOUND" && (
                <div><a onClick={() => nav.go("createBusinessAccount")} style={{ color: "var(--error-text)", fontWeight: 700, textDecoration: "underline" }}>Crear una cuenta</a></div>
              )}
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div>
            ...
            <a onClick={() => nav.go("forgotPassword")} style={{ display: "block", textAlign: "right", fontSize: 12.5, color: "var(--accent)", fontWeight: 600, marginTop: 10 }}>
        ...
        <p style={{ textAlign: "center", fontSize: 13, color: "var(--text-dim)", marginTop: 10 }}>
          ¿No tienes cuenta? <a onClick={() => nav.go("createBusinessAccount")} style={{ color: "var(--accent)", fontWeight: 600 }}>Regístrate</a>
        </p>
      </div>
    </div>
  );
